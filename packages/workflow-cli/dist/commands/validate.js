 ...
 */
function extractTemplateExpressions(obj) {
    const expressions = [];
 ...
 */
function validateTemplateExpression(expr, taskIds) {
 ...
 */
function validateTaskReferences(workflow, tasks, hasTasksPath) {
    if (!hasTasksPath) {
        return {
 ...
 */
function validateTemplateExpressions(workflow, taskIds) {
    const errors = [];
    // Check task inputs
    for (const task of workflow.spec.tasks) {
        if (task.input) {
 ...
 */
function validateCircularDependencies(workflow) {
 ...
 */
function validateDuplicateTaskIds(workflow) {
    const errors = [];
    const seen = new Set();
    for (const task of workflow.spec.tasks) {
        if (seen.has(task.id)) {
 ...
 */
function validateDependsOnReferences(workflow) {
    const errors = [];
