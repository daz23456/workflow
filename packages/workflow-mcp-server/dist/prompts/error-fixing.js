...
export const ERROR_FIXES = {
    unknown_task_ref: (taskRef, availableTasks) => `Task "${taskRef}" not found. Available tasks: ${availableTasks}. ` +
        `Please use one of the available task references.`,
    invalid_template: (template) => `Invalid template syntax: "${template}". ` +
    ...
    schema_mismatch: (field, expected, actual) => `Field "${field}" expects type ${expected} but got ${actual}. ` +
        `Check the task's input schema and provide the correct type.`,
    missing_dependency: (taskId, referencedTask) => `Task "${taskId}" references "${referencedTask}" in its input but doesn't list it in dependsOn. ` +
        `Add "${referencedTask}" to the dependsOn array.`,
    duplicate_task_id: (taskId) => `Duplicate task ID "${taskId}". ` +
        `Each task must have a unique ID within the workflow.`,
    invalid_yaml: (error) => `Invalid YAML syntax: ${error}. ` +
        `Check indentation and ensure all strings with special characters are quoted.`,
    missing_required_field: (field, location) => `Missing required field "${field}" in ${location}. ` +
 ...
 */
export function generateSuggestion(message, availableTasks) {
...
//# sourceMappingURL=error-fixing.js.map