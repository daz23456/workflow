import { detectErrorType } from '../prompts/error-fixing.js';
/**
 * Error types that can be automatically fixed
 */
const FIXABLE_ERROR_TYPES = [
    'unknown_task_ref',
    'invalid_template',
    'missing_dependency'
];
...
export class RefinementService {
    client;
    maxIterations;
    constructor(client, maxIterations = 3) {
        this.client = client;
     ...
     */
    async refineWorkflow(yaml, availableTasks) {
        const history = [];
        const seenErrorHashes = new Set();
     ...
     */
    generateFixes(errors, availableTasks) {
        const fixes = [];
     ...
     */
    generateFix(errorType, message, availableTasks) {
        switch (errorType) {
        ...
        for (const fix of fixes) {
            ...
            if (fix.startsWith('Replace taskRef')) {
                ...
                if (match) {
                    ...
                    result = result.replace(new RegExp(`taskRef:\\s*${this.escapeRegex(oldRef)}`, 'g'), `taskRef: ${newRef}`);
                }
            }
     ...
     */
    findClosestTask(invalidRef, tasks) {
        if (tasks.length === 0)
            return null;
        let bestMatch = null;
        let bestScore = 0;
     ...
     */
    addDependency(yaml, taskId, depToAdd) {
