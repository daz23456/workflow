...
import { buildSystemPrompt, formatTaskList } from '../prompts/system-prompt.js';
import { LlmService } from '../services/llm-service.js';

/**
 * Detect the workflow pattern from user intent
 */
export function detectPattern(intent: string): WorkflowPattern {
  const lowerIntent = intent.toLowerCase();

  // Check for parallel indicators
  if (
    lowerIntent.includes('at the same time') ||
  ...
  if (
    (lowerIntent.includes('check') && lowerIntent.includes('then')) ||
 ...
 */
export function parseGeneratedYaml(response: string): string {
 ...
 */
function toTaskSummary(task: WorkflowTask): TaskSummary {
...
function generateWorkflowFromTemplate(
  ...
  const { yaml, taskCount } = generateWorkflowYaml(
    intent,
    taskSummaries,
    pattern,
    maxTasks
  ...
  const explanation = `This workflow uses a ${pattern} pattern. ${
    pattern === 'sequential'
      ...
      : 'Tasks are organized in a complex pipeline.'
  } Generated ${taskCount} task(s) based on the intent: "${intent}".`;
