...
export function PropertiesPanel() {
  ...
  const renderSchemaProperties = (schema: SchemaDefinition | undefined, isRequired: (name: string) => boolean) => {
    if (!schema?.properties || Object.keys(schema.properties).length === 0) {
      return <p className="text-sm text-gray-500 dark:text-gray-400 italic">No properties defined</p>;
    }
        ...
        {Object.entries(schema.properties).map(([name, prop]) => (
          <div key={name} className="flex items-start gap-2 text-sm">
            <span className={cn(
              ...
              isRequired(name) ? "bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200" : "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300"
            )}>
              {name}
              {isRequired(name) && <span className="text-red-500 ml-0.5">*</span>}
            </span>
            <span className="text-gray-500 dark:text-gray-400">:</span>
            <span className="text-purple-600 dark:text-purple-400 font-mono text-xs">{prop.type}</span>
            {prop.description && (
              <span className="text-gray-500 dark:text-gray-400 text-xs truncate flex-1" title={prop.description}>
                - {prop.description}
              </span>
            )}
          </div>
  ...
  const ValidationIcon = () => {
    switch (validationState) {
      case 'error':
        return (
          <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded">
            <div data-testid="validation-error" className="flex-shrink-0">
              <AlertCircle className="w-5 h-5 text-red-500" />
            </div>
            <p className="text-sm text-red-700 dark:text-red-300">{getValidationMessage()}</p>
          </div>
        );
      case 'warning':
        return (
          <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded">
            <div data-testid="validation-warning" className="flex-shrink-0">
              <AlertTriangle className="w-5 h-5 text-yellow-500" />
            </div>
            <p className="text-sm text-yellow-700 dark:text-yellow-300">{getValidationMessage()}</p>
          </div>
        );
      case 'valid':
        return (
          <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded">
            <div data-testid="validation-valid" className="flex-shrink-0">
              <CheckCircle className="w-5 h-5 text-green-500" />
            </div>
            <p className="text-sm text-green-700 dark:text-green-300">{getValidationMessage()}</p>
          </div>
        );
    }
  };
  ...
  return (
    <div
      ...
      className="w-full h-full bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden"
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {!selectedNode ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">No Task Selected</h3>
          ...
          <div className="space-y-4">
            ...
            {selectedNodeIds.length > 1 && (
              <div className="p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded">
            ...
            >
              <input
                ...
                readOnly
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-900 text-gray-600 dark:text-gray-400 cursor-not-allowed"
            ...
            {hasInputProperties && taskDetails && !taskDetailsLoading && (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Input Configuration</h3>
                </div>
                ...
                {getMissingRequiredInputs.length > 0 && (
                  <div
                    ...
                    className="p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded text-sm text-red-700 dark:text-red-300"
                  >
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="text-gray-400 dark:text-gray-500 text-xs ml-2">({property.type})</span>
                        </label>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className={cn(
                              ...
                              isTemplate
                                ? "border-purple-300 dark:border-purple-700 bg-purple-50 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200 font-mono text-sm"
                          ...
                          {isTemplate && !showSuggestionsForField && (
                            <div
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/50 px-1.5 py-0.5 rounded"
                            >
                              Template
                            </div>
                          )}
                            ...
                            >
                              ...
                              {templateSuggestions.map((suggestion) => (
                                <button
                                  ...
                                  type="button"
                                  className="w-full px-3 py-2 text-left text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 focus:bg-blue-50 dark:focus:bg-blue-900/30 focus:outline-none font-mono text-purple-700 dark:text-purple-300"
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4 space-y-4">
                ...
                {taskDetails && !taskDetailsLoading && (
                  <>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <ArrowRightToLine className="w-4 h-4 text-green-600 dark:text-green-400" />
                        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Input Schema</h3>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <ArrowRightFromLine className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Output Schema</h3>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3">
                        {renderSchemaProperties(
